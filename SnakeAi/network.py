...
class Network(nn.Module):
    def __init__(self, n_output=3, lr_rate=1e-3):
        super(Network, self).__init__()
        self.conv1 = nn.Conv2d(2, 2, kernel_size=(2, 2), stride=(1, 1))
        self.conv2 = nn.Conv2d(2, 2, kernel_size=(2, 2), stride=(1, 1))
        self.linear1 = nn.Linear(128, 10)
        self.linear2 = nn.Linear(128, 10)
        self.linear3 = nn.Linear(10, n_output)
...
class Agent:
    def __init__(self, network_model, gamma, batch_size, n_action, input_dims, mem_size=100000):
        ...
        self.q_memory = np.zeros(mem_size, dtype=np.float32)
    ...
    def store_data(self, state, action, reward, new_state, done):
        ...
        self.terminal_memory[index] = 1 if done else 0
        self.mem_ctr += 1
    ...
    def choose_action(self, observation):
        ...
        action = T.multinomial(
            T.softmax(self.network_model(state).to(self.network_model.device), dim=1, dtype=T.float32), 1)[0]
        return action.item()

    def learn(self, start, stop):
        ...
        q_eval = self.network_model(state_batch)[batch_index, action_batch]
        q_next = self.network_model(new_state_batch)
